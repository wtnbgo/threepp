...
/* [krkreffekseer 独自変更]
 * 以下は libGLESv2 を自前で dlopen/LoadLibrary して EGL からエントリポイントを
 * 引く「スタンドアロンローダ」(gladLoaderLoadGLES2)。
 * 本プラグインは吉里吉里が既に張った GL コンテキストへ相乗りするため、
 * これを呼ぶと EGL 接続が二重になり誤動作する。使うのは
 * gladLoadGLES2(loadfunc) のみなので、GLAD_NO_STANDALONE_LOADER を定義して
 * ブロックごと落とす (glad/egl.h への依存も不要になる)。 */
#if defined(GLAD_GLES2) && !defined(GLAD_NO_STANDALONE_LOADER)
